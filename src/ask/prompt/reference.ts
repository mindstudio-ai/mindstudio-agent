...
export function buildReferenceDocs(data: ReferenceData): string {
  ...
  Uploading files / getting a hosted URL: There are TWO ways to upload a file to the MindStudio CDN, depending on context. (1) From app backend code, call \`await mindstudio.uploadFile(bytes, { extension: 'png', type?: 'image/png' })\` → returns \`{ url }\` (a permanent public URL); \`bytes\` is a \`Buffer\`/\`Uint8Array\`, so read/fetch the content first. (2) From the terminal — e.g. you generated an icon or asset locally and just need its URL — run \`mindstudio upload <filepath>\` (alias: \`mindstudio upload-file\`). It prints the public URL to stdout. IMPORTANT: to host a one-off local file, use the CLI command directly — do NOT write a throwaway app method that calls \`uploadFile\` just to trigger it. The CLI does this in one step. More generally: some SDK capabilities are exposed as \`mindstudio\` CLI commands (upload, list-actions, run-agent, exec, batch, …), not only as code — reach for the CLI for one-off operations instead of writing and running code.
