...
export function buildReferenceDocs(data: ReferenceData): string {
  ...
  Database aggregation: \`count()\`, \`sum(fn)\`, \`avg(fn)\`, \`countDistinct(fn)\`, and \`aggregate(spec)\` compile to SQL aggregates (COUNT/TOTAL/AVG/GROUP BY) — no rows are fetched, so they stay cheap on tables of any size. NEVER page a whole table into memory to compute a summary; aggregate in the database. Grouped example (the spec uses STRING column names, and the filter uses bindings so it compiles to SQL):
  \`\`\`typescript
  ...
  \`\`\`
  Select terms: \`{ count: true }\`, \`{ sum: 'col' }\`, \`{ avg: 'col' }\`, \`{ min: 'col' }\`, \`{ max: 'col' }\`, \`{ countDistinct: 'col' }\`. Omit \`by\` for whole-set aggregates (returns a single object). Empty-set semantics: count/countDistinct 0, sum 0, avg/min/max null. \`Table.min/max(fn)\` still return the full ROW with the min/max value; \`groupBy(fn)\` returns full rows per group and fetches everything — prefer \`aggregate()\` for summaries.

  Raw SQL escape hatch (\`db.sql\`): \`await db.sql<T>('SELECT ... FROM t WHERE x = ? ...', [val])\` runs read-only raw SQL against the app's own managed database — for joins, subqueries, and window functions the typed API can't express. SELECT/WITH only (writes throw — use Table methods). Lazy and batchable via \`db.batch()\`. LAST RESORT: prefer the typed API (including \`aggregate()\`) whenever it can express the query; raw rows may not exactly match the typed API's value representations.
  ...
  Task agents: For multi-step tasks requiring autonomous tool use, use \`runTask()\`. Provide a prompt, input, a set of tools, an output contract, and a model. The SDK runs a tool-use loop and returns structured output. Prefer \`outputSchema\` for the output contract: plain JSON Schema (\`type\`/\`properties\`/\`required\`/\`enum\`/\`items\`; nullable via type arrays like \`["string","null"]\` — never \`nullable: true\`, never \`oneOf\`/\`$ref\`). Output is validated every turn with automatic repair and \`result.output\` is typed by inference from the schema — no generic argument and no \`parsedSuccessfully\` check needed; the call throws \`MindStudioError\` code \`task_output_schema_mismatch\` instead of ever returning nonconforming output. The legacy \`structuredOutputExample\` option (example object the model imitates) still works but is unvalidated — always check \`result.parsedSuccessfully\` before using its output. \`result.traceId\` is a stable id for the task's model transcript — inside a jewel, attach it to the propose/grade return (\`trace: task.traceId\`) to preserve the transcript with the pair. Tools can include any SDK action — e.g. \`['searchGoogle', 'fetchUrl', { method: 'generateImage', defaults: { imageModelOverride: { model: 'seedream-4.5' } } }]\` — plus the app's own methods (\`{ appMethod: 'save-vendor', description: 'when to use it' }\`, run as the user who invoked the method that started the task) and inline functions (\`{ name, description, inputSchema, execute }\` — the function runs in the calling process; thrown errors go back to the model as tool output; no \`defaults\`). Supports SSE streaming via \`onEvent\` callback.
  ...
  Reading the app's own traffic (\`analytics\`): The platform auto-tracks pageviews (and custom events via the frontend's \`analytics.track()\`); the backend \`analytics\` namespace is the read side, so app code can correlate real traffic with its own data (lifetime views next to each blog post, "N people reading now"). One general read — \`analytics.query({ metrics, dimensions?, filters?, dateRange?, granularity?, orderBy?, limit? })\` — shaped like the Plausible query API: metrics from \`pageviews | visitors | visits | events\`, at most ONE entity dimension (path, referrerHost, country, deviceType, eventName, ...) OR \`'time'\` (+ \`granularity\`), filter triples \`[['is'|'is_not'|'contains', dimension, values]]\`, \`dateRange\` shorthand (\`'7d'\`, \`'all'\`, or \`[startISO, endISO]\`). Example — lifetime views per page: \`await analytics.query({ metrics: ['pageviews', 'visitors'], dimensions: ['path'], dateRange: 'all', limit: 10 })\` → \`results[].{ dimensions: { path }, metrics: { pageviews, visitors } }\`. IMPORTANT retention split: queries whose filters are all \`is\` and touch at most one dimension read a rollup kept forever (full history, exact uniques); cross-dimension / \`is_not\` / \`contains\` queries scan raw events retained 90 days — the server clamps the window and reports it (\`meta.source\`, \`meta.clamped\`); metrics a shape can't carry appear in \`meta.metricsOmitted\`, never as silent zeros. \`city\` filter values are plain city names (any country); with \`dateRange: 'all'\` the granularity is a minimum the server coarsens as history grows. Specials: \`analytics.live()\` (current visitor count + countries + sparkline), \`analytics.sources()\` (per-session first-source ranking, classified incl. AI assistants), \`analytics.map()\`, \`analytics.aiSources()\`, \`analytics.crawlers.overview()/timeseries()/recent()\`. Backend only (hook token); reads the LIVE app's real traffic even from a dev sandbox. Rate limited per app (600 reads/min; the SDK auto-retries a 429 once). For a page composed of several reads (KPIs + timeseries + breakdowns), use \`analytics.batch([spec, ...])\` (≤10 queries, one round trip, one rate token each, results in order) instead of a parallel burst of \`query()\` calls. NEVER swallow \`rate_limited\` errors into empty/zero data — surface the error or retry; a page silently rendering zeros is worse than one showing a retry state.

  Email (\`email\` namespace — sending AND the app's whole delivery console): For app code, ALWAYS prefer the top-level \`email\` namespace over the legacy \`sendEmail\` action: \`import { email } from '@mindstudio-ai/agent'\` (also on the agent instance). \`email.send({ to, subject, body, category?, batchId?, ... })\` takes the same fields as the action and returns \`{ recipients, suppressed, batchId }\`. The \`sendEmail\` ACTION is the legacy v1-workflow path — still supported and running the same underlying send, but only reach for it when you need \`connectionId\` (OAuth-resolved recipients) or \`generateHtml\`; never present it as the default way to send. Key semantics: \`category\` defaults to \`'marketing'\` (per-recipient one-click unsubscribe header; unsubscribed recipients are SKIPPED and listed in \`suppressed\` — a non-empty \`suppressed\` is a success, not an error; cc/bcc are flattened into individual messages); pass \`'transactional'\` for receipts, alerts and codes (no unsubscribe header, real cc/bcc semantics, opt-outs ignored). Large and marketing sends are QUEUED: \`send()\` resolves once the send is ACCEPTED and delivery continues in the background — read progress and outcomes with \`email.batch(batchId)\` (\`status: accepted|sending|completed|partial|failed\` plus \`pending\`/chunk progress; \`status: null\` means an inline send that already finished — normal, not an error). Errors worth branching on: \`outbound_daily_cap_exceeded\` (429 — the app's daily recipient quota; check \`email.quota()\` before a big send and split the list) and \`sending_paused\` (409 — sending is paused for this app, for reputation or by support; retrying will not help). The rest of the namespace is the full observability surface — an app can build its own email admin screen and never open our dashboard: \`email.stats()\` (counts, rates, timeseries; below ~50 accepted show counts rather than percentages), \`email.messages({ status?, kind?, recipient?, batchId?, search? })\` (the per-recipient log INCLUDING mail that never reached the provider — unsubscribed, over-cap, sender-not-allowed — which is where most "it didn't send" reports land), \`email.message(id)\` (bounce diagnostics), \`email.batches()\`, and \`email.suppressions()\` / \`suppress(addr)\` / \`unsuppress(addr)\` (after an unsuppress, check \`platformSuppression\` on the result before telling anyone mail will now reach them). Backend only (hook token).
