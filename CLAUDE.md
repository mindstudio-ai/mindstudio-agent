...
- **Jewels** (`defineJewel`) are hand-written in `src/jewel/index.ts`. A jewel is an agentic shadow companion to an app method, authored in `foo.jewel.ts` beside `foo.ts`; `defineJewel(method, { subject, propose, grade? })` returns a **callable executor with the config attached as properties** (Express-app pattern) so the sandbox worker's `mod[handlerName](params)` invokes a jewel run as an ordinary method-execution frame — zero worker/protocol changes. Params are exactly one of `{ humanInput }` (shadow: subject derived via the projection, humanInput doubles as ground truth) or `{ subject }` (eval: ungraded). `subject`/`propose` throwing never propagates (captured as `error` on the record); `grade` throwing softens to verdict `'skip'`; the default grade is deep-equal on method inputs (JSON semantics: `undefined`-valued keys treated as absent; both-null → agree). Output is a versioned JSON-serializable `JewelPairRecord` (`v: 1`, mode, subject, proposed/reasoning, actual, verdict/notes, error, timing). Attached properties for the compiler: `kind: 'jewel'`, `method` (reference identity for the binding check), `subject`, `propose`, `grade` (`undefined` = default grade). **Not yet executed by the platform** — deliberately no README/llms.txt/ask-agent teaching yet; reference consumers are the own-the-learning working doc (youai-api repo) and the error-triage demo app.
