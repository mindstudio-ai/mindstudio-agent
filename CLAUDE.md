...
src/
  ...
  task/
    index.ts            # Legacy whole-task transport (poll + SSE), tool mapping, alias resolution
    local.ts            # The task loop — SDK-owned, per-turn transport via POST /task/turn, retries, tool dispatch
    schema.ts           # outputSchema dialect: JsonSchema/FromSchema types, runtime validator, skeleton-example generator
    types.ts            # RunTaskOptions (example/schema variants), RunTaskResult, TaskEvent, TaskUsage, TaskToolConfig
  jewel/
    index.ts            # defineJewel — agentic shadow companion for an app method (callable executor + attached config)
...
- **Streaming debug logs.** `StepExecutionOptions.onLog` enables SSE streaming from the step execution endpoint. When set, the SDK sends `Accept: text/event-stream`, parses log/done/error events, fires callbacks for log events, and resolves the promise on done. Works on ALL step methods (generateImage, generateVideo, etc. — not just generateText). The CLI automatically enables streaming on TTY — log events render to stderr while the result goes to stdout.
- **`stream()` — frontend SSE push.** Sends real-time updates from method code to the frontend. `stream('text')` sends a `token` event (frontend `onToken` receives accumulated text, not deltas). `stream({ ... })` sends a `data` event (frontend `onStreamData` receives structured data). These are different event types — do not mix them up. Silently no-ops when no SSE connection is active. Standard pattern for long-running operations: use `onLog` + `stream()` together to forward step progress to the UI.
...
- **Task agents** (`runTask`) are hand-written on `MindStudioAgent`. The tool-use loop runs **client-side** in `src/task/local.ts`, using stateless `POST /developer/v2/task/turn` as a per-turn transport (tasks survive API pod recycles); if that route 404s (old/self-hosted server) it falls back to the legacy whole-task `POST /developer/v2/task` (poll or SSE, in `src/task/index.ts`). The model receives the developer's prompt and a set of SDK actions, app methods, and/or inline function tools (`{ name, description, inputSchema?, execute }` — executed client-side in the loop, definition-only on the wire; no `defaults`; unsupported on the legacy fallback, which throws `task_function_tools_unsupported` rather than silently dropping them), calls them as needed, and produces structured JSON output. Two output modes: **`outputSchema`** (preferred — plain JSON Schema in the tool-definition dialect, subset enforced by `assertSupportedSchema`; output validated every turn with bounded repair turns, `result.output` typed via `FromSchema` inference from the schema literal, throws `MindStudioError` `task_output_schema_mismatch` instead of ever resolving with nonconforming output) or the legacy **`structuredOutputExample`** (example-shaped, unvalidated — callers must check `parsedSuccessfully`). SDK-action tools are method names with optional default overrides — aliases are resolved via `resolveStepType()` (app-method and function entries deliberately bypass it). Types: `RunTaskOptions` (+ `RunTaskOptionsWithSchema`/`RunTaskOptionsWithExample`), `RunTaskResult`, `TaskEvent`, `TaskUsage`, `TaskToolConfig`, `JsonSchema`, `JsonObjectSchema`, `FromSchema`, `SchemaValidationError`. Implementation in `src/task/`.
- **Jewels** (`defineJewel`) are hand-written in `src/jewel/index.ts`. A jewel is an agentic shadow companion to an app method, authored in `foo.jewel.ts` beside `foo.ts`; `defineJewel(method, { subject, propose, grade? })` returns a **callable executor with the config attached as properties** (Express-app pattern) so the sandbox worker's `mod[handlerName](params)` invokes a jewel run as an ordinary method-execution frame — zero worker/protocol changes. Params are exactly one of `{ humanInput }` (shadow: subject derived via the projection, humanInput doubles as ground truth), `{ subject }` (eval/arrival: ungraded — arrival proposals are graded later), or `{ grade: { proposed, actual } }` (grade-only: deferred grading of an arrival proposal, running the jewel's own `grade` or the default). `subject`/`propose` throwing never propagates (captured as `error` on the record); `grade` throwing softens to verdict `'skip'`; the default grade is deep-equal on method inputs (JSON semantics: `undefined`-valued keys treated as absent; both-null → agree — the platform mirrors this exactly in `deepEqualJson`, keep in sync). Output is a versioned JSON-serializable `JewelPairRecord` (`v: 1`, mode, subject, proposed/reasoning, actual, verdict/notes, error, `customGrade`, timing). Attached properties for the compiler: `kind: 'jewel'`, `method` (reference identity for the binding check), `subject`, `propose`, `grade` (`undefined` = default grade). **`mindstudio.jewels.propose(methodId, subject, { idempotencyKey? })`** is the arrival-shaped trigger: hook-token only (raw fetch like `reportIssue`, shared `_jewelsRequest` helper), Stripe replay semantics (a replayed open key returns the ORIGINAL outcome), outcome union in `JewelProposeOutcome` (`queued` carries `queueItemId`) — the call blocks for the jewel run, so app chains wrap in `mindstudio.waitUntil(...)`. **`mindstudio.jewels.queue.list/resolve`** is the app-native approval surface for `approve`-mode methods: list returns pending items (a queue item is an open pending proposal platform-side, no separate table); resolve `approve` applies the target method AS THE CURRENT SESSION USER (the reviewer — the target method's own auth gates who may approve; an edited `input` is captured as resolution `edited` with proposed/final on the pair), `dismiss` records and closes without running anything.
- **Issue reporting** (`reportIssue`) is hand-written on `MindStudioAgent`. Lets an app's backend file a bug/idea into the app's issue tracker (for in-app "report a bug" features). Raw hook-token `POST /_internal/v2/report-issue` (mirrors `_executeDbBatch` — `Authorization: this._token`, no `Bearer`; `appId` derived from the token, not sent). **Backend/managed-context only** — an API-key caller outside the sandbox gets 401. Deliberately does NOT route through `request()` (which auto-retries 429): a per-app rate limit (20/60s) must surface immediately as a `MindStudioError` with `code === 'rate_limited'`. Parses the `{ code, errorString, errorMessage }` envelope, mapping `errorString` → `MindStudioError.code`. Returns the full `ReportedIssue`. Input `{ title, body?, kind?, reporter? }` maps 1:1 to the wire (`reporter` is a free-form display label, not an identity). Types: `ReportIssueInput`, `ReportedIssue`. **Not exposed via CLI or MCP** (those run with API keys, not hook tokens).
