...
function generateLlmsTxt(steps: StepInfo[]): string {
  ...
  lines.push(
    '- **Task agents**: For multi-step tasks that need autonomous tool use (research, content creation, data enrichment), use `runTask()`. Provide a prompt, input, SDK action names as tools, a structured output example, and a tool-use model id. The platform runs a tool-use loop — calling the model, executing actions, feeding results back — until structured output is produced. Example: `runTask({ prompt: "Find info about this restaurant", input: { name: "Tartine" }, tools: ["searchGoogle", "fetchUrl"], structuredOutputExample: \'{"name":"...","url":"..."}\', model: yourModelId })`. Pass a current tool-use model id (call `listModels` or ask). Don\'t hardcode one.',
  );
  lines.push('');
